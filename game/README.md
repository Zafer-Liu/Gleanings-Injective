...
开发服务器和生产构建都会安装调试入口。进入游戏后打开浏览器 DevTools 的
Console，输入 `dev`（不带括号）即可显示调试面板并启用隐藏快捷键：

- `Alt+Shift+N`：直接跳到下一关，并自动补齐可加载的前置存档
- `Alt+Shift+B`：解锁收藏馆中的全部徽章
- `Alt+Shift+I`：解锁收藏馆中的全部道具

三项操作也可以在调试面板中一键执行。关闭面板后再次输入 `dev` 即可唤出。
也可以在主页先输入 `dev`，进入游戏后面板会自动显示。
调试收藏使用独立的
`gleanings.debug.collectibles.v1` 本地标记，不会改写正常剧情进度。
若 Console 的执行上下文不是页面顶层，可改为输入 `window.dev`。
