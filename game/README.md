...
调试工具只会在 `pnpm dev` 的开发环境安装，生产构建不会启用。

- `Ctrl+Shift+D`：显示或隐藏调试面板
- `Alt+Shift+N`：直接跳到下一关，并自动补齐可加载的前置存档
- `Alt+Shift+B`：解锁收藏馆中的全部徽章
- `Alt+Shift+I`：解锁收藏馆中的全部道具

后三项也可以在调试面板中一键执行。调试收藏使用独立的
`gleanings.debug.collectibles.v1` 本地标记，不会改写正常剧情进度。
...
4. 《归家》：林念安和米娅一起整理三代人的文化标签，并将旅程收进酒签。
